...
package be.woutdev.tribes.squirrelid.resolver;

import be.woutdev.tribes.squirrelid.Profile;
import be.woutdev.tribes.squirrelid.util.HttpRequest;
import be.woutdev.tribes.squirrelid.util.UUIDs;
...
public class HttpRepositoryService implements ProfileService
{
    ...
    /**
     * Create a new resolver. <p> <p>For Minecraft, use the {@link #MINECRAFT_AGENT} constant. The UUID to name mapping
     * is only available if a user owns the game for the provided "agent," so an incorrect agent may return zero results
    ...
    /**
     * Get the number of milliseconds to delay after each failed HTTP request, doubling each time until success or total
     * failure.
     *
    ...
    /**
     * Set the number of milliseconds to delay after each failed HTTP request, doubling each time until success or total
     * failure.
     *
